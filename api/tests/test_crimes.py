...
def test_list_crimes_filter_by_crime_type(client):
    res = client.get("/api/v1/crimes/?crime_type=Burglary")
    assert res.status_code == 200
    data = res.json()
    assert data["total"] == 2
...
def test_list_crimes_filter_by_month(client):
    res = client.get("/api/v1/crimes/?month=2026-01")
    assert res.status_code == 200
    assert res.json()["total"] == 1


def test_list_crimes_pagination(client):
    res = client.get("/api/v1/crimes/?page=1&page_size=2")
    assert res.status_code == 200
    data = res.json()
...
def test_list_crimes_page_size_limit(client):
    res = client.get("/api/v1/crimes/?page_size=501")
    assert res.status_code == 422


def test_cursor_pagination_first_page(client):
    res = client.get("/api/v1/crimes/cursor?limit=2")
    assert res.status_code == 200
    data = res.json()
...
def test_cursor_pagination_second_page(client):
    first = client.get("/api/v1/crimes/cursor?limit=2").json()
    cursor = first["next_cursor"]
    second = client.get(f"/api/v1/crimes/cursor?limit=2&cursor={cursor}").json()
...
def test_get_crime_not_found(client):
    res = client.get("/api/v1/crimes/99999")
...
def test_summary_unknown_force(client, auth_headers):
    res = client.get("/api/v1/crimes/summary?force=Unknown Force XYZ", headers=auth_headers)
    assert res.status_code == 200
